...
class FilterForm extends Component {
    ...
    smartSearchFilter = async (value) => {
        if (value.length > 2) {
            // if (type === 'user') {
            this.props.dispatch(UserSearchAction.searchUsers({
                field: "full_name",
                value: value
            }));
            // }
    ...
    loadOptions = async (inputValue, callback) => {
        ...
        const result = data.map(d => {
            return { value: `${d.first_name} ${d.last_name}`, label: `${d.first_name} ${d.last_name}` }
        })
        callback(result);
    };
    ...
    render() {
        ...
        const invoice_customer_name = this.props.invoices.map(function (invoice) {
            ...
            <React.Fragment>
                <form>
                    <Form.Row className="align-items-center mb-4">
                        ...
                        {this.props.fields.indexOf("site_customer_name") !== -1 ? (
                            <Col md="3 mb-3">
                                <InputSelectField
                                    name="fullname"
                        ...
                        {this.props.fields.indexOf("invoice_customer_name") !== -1 ? (
                            <Col md="3 mb-3">
                                <InputSelectField
                                    name="full_name"
